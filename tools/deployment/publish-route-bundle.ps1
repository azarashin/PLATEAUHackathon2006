...
$serverUser = Get-DeploymentSetting 'ROUTE_DEPLOY_USER'
...
if ([string]::IsNullOrWhiteSpace($serverUser) -or $serverUser -notmatch '^[A-Za-z0-9._-]+$') { throw 'ROUTE_DEPLOY_USER is required and contains unsupported characters.' }
$sshPort = 0
...
if ([string]::IsNullOrWhiteSpace($remoteRoot) -or $remoteRoot -notmatch '^/[A-Za-z0-9._/-]+$') { throw 'ROUTE_DEPLOY_ROOT is required and must be an absolute Linux path without spaces or shell characters.' }
