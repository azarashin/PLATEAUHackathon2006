...
public sealed class EnvironmentCostRuntimeUiController : MonoBehaviour
{
    ...
    private EnvironmentCostRuntimeOverviewMapController overviewMap;
    ...
    private IEnumerator Start()
    {
        ...
        overviewMap = GetComponent<EnvironmentCostRuntimeOverviewMapController>() ?? gameObject.AddComponent<EnvironmentCostRuntimeOverviewMapController>();
        ...
        surface.AddToClassList("runtime-ui-surface");
        root.Add(surface);
        runtimeUiSurface = surface;
        EnvironmentCostRuntimeUiInputGate.TrackDocument(runtimeDocumentRoot, surface);
        var style = Resources.Load<StyleSheet>("EnvironmentCostRuntimeUi");
        if (style != null) root.styleSheets.Add(style);
        var tabs = new VisualElement(); tabs.AddToClassList("runtime-tabs"); surface.Add(tabs);
        var content = new VisualElement(); content.AddToClassList("runtime-tab-content"); surface.Add(content);
        solar.BuildUi(content);
        shade.BuildUi(content);
        policy.BuildUi(content);
        comparison.BuildUi(content);
        heatmap.BuildUi(content);
        overviewMap.BuildUi(root);
    ...
    private void Update()
    {
        ...
        var isPointerOverUi = runtimeUiSurface.worldBound.Contains(panelPosition) || overviewMap != null && overviewMap.IsPointerOverMap;
