...
Issue #8で追加した時間別キャッシュ、完了状態、欠測理由、結果フィンガープリント、Unityヒートマップを含む現行仕様と再実行結果は、[時間別環境コストの解析・検証・可視化](hourly-environment-cost-analysis.md)を参照する。本資料前半の所要時間・出力サイズ・SHA-256は、Issue #2時点の旧`environment-cost-0.1`出力の記録として残す。
