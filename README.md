...
Viewer は正式データ契約 v1 の小型 fixture を直接読み込みます。MapLibre の地図上で日陰・内水コストを切り替え、道路色、凡例、説明、サンプル KPI の連動と欠測表示を確認できます。市ヶ谷では Unity 解析と歩行道路グラフ生成まで実施済みで、実解析結果との結合は後続 Issue の対象です。
