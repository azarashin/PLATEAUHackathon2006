...
Viewer は正式データ契約 v1 の小型 fixture による環境コスト表示に加え、5地域の選択、GPSによる表示位置合わせ、地図クリックによる起終点指定、計算済み日時の選択を実装しています。市ヶ谷では経路サーバーへ検索条件を送り、同じ規則で道路へスナップできます。経路線とKPI比較の実応答描画はIssue #13の対象です。
