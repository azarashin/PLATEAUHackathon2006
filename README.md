...
Viewer は正式データ契約 v1 の小型 fixture による環境コスト表示に加え、5地域の選択、GPSによる表示位置合わせ、地図クリックによる起終点指定、計算済み日時の選択を実装しています。市ヶ谷では経路サーバーへ検索条件を送り、道路へスナップした最短・バランス・日陰優先の3経路とKPIを実応答から描画します。
...
日陰率は、指定時刻に道路サンプルが直射日光を受けるかを示す解析指標です。体感温度を推定するものではなく、安全を保証するナビゲーションとして使用してはいけません。実演時の固定条件、性能値、異常時の復旧手順は [市ヶ谷 E2E デモ運用手順](docs/e2e-demo-runbook.md) を参照してください。
